...
#if defined(NDEBUG)
# error "Dogegold cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Dogegold Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetDogegoldMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Dogegold: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("dogegold-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Dogegold: BIP30 has been active since inception
    ...
    // Dogegold: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetDogegoldBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Dogegold: Disallow AuxPow blocks before it is activated.
    ...
    // Dogegold: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Dogegold: We probably want to disable this
    int nLockTimeFlags = 0;
