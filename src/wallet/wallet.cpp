...
{
    // Dogegold: Add an increased fee for each dust output
    return std::max(minTxFee.GetFee(nTxBytes) + GetDogegoldDustFee(tx.vout, minTxFee), ::minRelayTxFee.GetFee(nTxBytes));
}
...
CAmount CWallet::GetMinimumFee(const CMutableTransaction& tx, unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool, CAmount targetFee)
{
    ...
    // Dogegold: Drop the smart fee estimate, use GetRequiredFee
